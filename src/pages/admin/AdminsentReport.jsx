...
const notAcknowledgePercentage = (((memoDetails.recipients.length - acknowledgedCount) / memoDetails.recipients.length) * 100).toFixed(2);
  ...
  const renderCustomLabel = (props) => {
    const { cx, cy, midAngle, innerRadius, outerRadius, percent, index, position, labelTop, labelBottom } = props;
    ...
    if (position === 'bottom') {
      return (
        <text x={cx} y={cy + outerRadius + 20} textAnchor="middle" fill="#333">
          {labelBottom}
        </text>
      );
    }
